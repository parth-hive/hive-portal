...
- `src/app/(app)/**` — the authenticated portal. One folder per feature (`tenants`, `properties`, `inventory`, `cleaning`, `reconciliation`, `agreements`, `credentials`, `reports`, `settings`). Each typically has `page.tsx` (Server Component for reads) + `actions.ts` (`"use server"` mutations) + colocated client components.
