...
- `src/app/api/**` — `telegram/route.ts` (bot webhook), `cron/{rent-reminders,notification-followups}/route.ts` (Vercel cron, guarded by `CRON_SECRET`), `inventory/**` (read-only inventory API for internal scripts, bearer-token guarded by `INVENTORY_API_KEY`; core listing data only — no tenants/ads/listing actions), and `tenants/route.ts` (read-only tenant contact API for the text-responder bot — name/email/phone/unit/room with `?phone=`/`?q=` lookup; guarded by its own `TENANTS_API_KEY` since it returns PII). API docs live on the public `/developers` page (whitelisted in `proxy.ts` like `/docs`; content components in `src/components/docs/`, shared with the public `/docs/inventory-api` page — example payloads must stay fictional since these pages need no login).
...
See `.env.example`. Required: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`. Feature-gated: Resend (`RESEND_*`), Gmail (`GMAIL_*`), MS Graph (`MS_*`), Telegram (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `ALLOWED_TELEGRAM_USER_IDS`), `ANTHROPIC_API_KEY`, cron (`CRON_SECRET`), inventory API (`INVENTORY_API_KEY`), tenants API (`TENANTS_API_KEY`), `NEXT_PUBLIC_SITE_URL`, `SIGN_ORIGIN` (neutral no-"hive" domain for lease-signing links; NY sends refuse without it, and that host serves only `/sign/*`).
